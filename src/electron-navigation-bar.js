...
const enav = new (require('electron-navigation'))({
					...
					newTabCallback: (url, options) => { //only allow one electron tab open
										console.log(oneTab);
										if (oneTab){
											oneTab = false;
											return {url};
...
const navDefaultURL = "file:///menu/index.html";
...
addListenerMulti(document,["keydown","mousedown","mouseup","click","contextmenu","dblclick"], function (e) {
	var ctrlPressed = 0;
	var altPressed = 0;
	var shiftPressed = 0;
	...
	//the only one that works is DevTools and Close tab
	//disabled close tab function
	...
	if (ctrlPressed && fromKeyCode(evt.keyCode) == 'W'){
		//enav.closeTab(); stops user from closing only tab
		e.preventDefault();        
		return false;
	}
	// Print tab => Ctrl+P
